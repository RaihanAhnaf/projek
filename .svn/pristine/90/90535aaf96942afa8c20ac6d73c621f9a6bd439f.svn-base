...
ar.dataSupplier= ko.observableArray([])
ar.SupplierCode= ko.observable("")
...
ar.renderGridSummary = function(){
    ...
    $('#gridAr').kendoGrid({
        ...
        excel: {
            fileName: "report-apaging-summary.xlsx"
        },
        pdf: {
            allPages: true,
            paperSize: "A4",
    ...
    ajaxPost(url, param, function(res){
        ...
        for(i in data){
            if (newdata.length==0){
                ...
                var find = _.find(newdata, function(o) { return o.SupplierCode == data[i].SupplierCode });
                if(find!=undefined){
                    for (j in newdata){
                        if(data[i].SupplierCode ==newdata[j].SupplierCode ){
...
ar.renderGridDetail = function(){
    ...
    var columns = [
        {
            ...
            field : "PurchaseDate",
            template: function (e) {
                return moment(e.PurchaseDate).format("DD MMM YYYY")
            },
        },{
            title : "Due Date",
            field : "Due_Date",
    ...
    $('#gridAr').kendoGrid({
        ...
        excel: {
            fileName: "report-apaging-detail.xlsx"
        },
        sortable: true,
        scrollable: true,
        columns: columns,
...
ar.ExportToPdf = function(){
    ...
    ajaxPost("/report/exportpdfap", param, function (e) {
        model.Processing(false)
