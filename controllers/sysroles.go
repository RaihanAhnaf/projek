...
func (d *SysRolesController) SaveData(r *knot.WebContext) interface{} {
	...
	for _, det := range oo.Menu {
		...
		dtl.Parent = det["parentid"].(string)
	...
	if oo.Id == "" {
		d.LogActivity("Roles", "Save Data Roles", oo.Name, r)
	} else{
		d.LogActivity("Roles", "Update Data Roles", oo.Name, r)
	}
	return ret
}
...
func (d *SysRolesController) GetLandingPage(r *knot.WebContext) interface{} {
	...
	csr, err := d.Ctx.Connection.NewQuery().Select("Title").From("SysMenus").Cursor(nil)
